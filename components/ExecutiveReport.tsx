...
const ExecutiveReport: React.FC<ExecutiveReportProps> = ({ results, config, onReset, onReaudit, onRepeatAudit }) => {
  ...
  const ReportTabButton: React.FC<{ 
    ...
    <button
        onClick={onClick}
        className={`relative flex flex-col items-center gap-1 px-3 py-2.5 font-semibold text-xs transition-all min-w-[100px] ${
            active 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-slate-900 p-4 sm:p-6 lg:p-8">
      {/* Header con mejor diseño - Ancho optimizado */}
      <div className="max-w-4xl mx-auto mb-6 sm:mb-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">
              Análisis completo de {results.length} conversacion{results.length > 1 ? 'es' : ''}
            </p>
          </div>
        ...
        <div className="flex gap-1 border-b-2 border-gray-300 dark:border-gray-600 overflow-x-auto bg-gray-100 dark:bg-gray-800 rounded-t-xl p-1 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent">
          ...
          <ReportTabButton 
            ...
            icon="🎯" 
            label="Objetivos"
            count={results.filter(r => (r.analysis?.overallScore || 0) < 7).length}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-b-2xl rounded-tr-2xl shadow-2xl border-2 border-t-0 border-gray-300 dark:border-gray-600 p-6 sm:p-8 min-h-[600px] max-h-[80vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className={`bg-gradient-to-br ${colors.bg} rounded-2xl p-6 sm:p-8 border-2 ${colors.border} transition-all duration-300`}>
        ...
        <div className="bg-gray-50 dark:bg-gray-700/30 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Métricas de Auditoría</h2>
          </div>
        ...
        {costSummary && costSummary.totalCostUSD > 0 && (
          <div className="bg-gradient-to-r from-yellow-50 via-amber-50 to-orange-50 dark:from-yellow-900/20 dark:via-amber-900/20 dark:to-orange-800/20 rounded-2xl p-6 border-2 border-yellow-300 dark:border-yellow-600">
            ...
            {/* OBJECTIVES TAB - Cumplimiento de Objetivos */}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.map((result) => {
                    ...
                    const achieved = score >= 7;
                    return (
                      <div key={result.id} className={`rounded-xl p-6 border-2 ${achieved ? 'bg-green-50 dark:bg-green-900/20 border-green-400' : 'bg-red-50 dark:bg-red-900/20 border-red-400'}`}>
                        <div className="flex items-start gap-3 mb-3">
                          <span className="text-4xl">{achieved ? '✅' : '❌'}</span>
                          <div className="flex-1">
                            <h4 className="font-bold text-lg text-gray-900 dark:text-white">{result.testCase.title}</h4>
                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{result.testCase.conversationGoal}</p>
                          </div>
                        </div>
                        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-300 dark:border-gray-600">
                          ...
                          <span className={`text-2xl font-bold ${score >= 8 ? 'text-green-600' : score >= 5 ? 'text-yellow-600' : 'text-red-600'}`}>{score.toFixed(1)}/10</span>
                        </div>
                        <div className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                          💬 {results.find(r => r.id === result.id)?.executionTrace.length || 0} turnos • 
                          🗄️ {results.find(r => r.id === result.id)?.databaseActivity?.changes?.length || 0} cambios BD
                        </div>
                      </div>
                    );
                  })}
            ...
            {/* PRICES TAB - Errores de Precios */}
            {activeTab === 'prices' && (
              <div className="space-y-4">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                  <span className="text-3xl">💰</span>
                  Análisis de Precios vs Base de Datos
                </h3>
                {stats.totalDbErrors === 0 ? (
                  <div className="bg-green-50 dark:bg-green-900/20 rounded-2xl p-12 text-center border-2 border-green-400">
                    <div className="text-8xl mb-4">✅</div>
                    <h4 className="text-2xl font-bold text-green-800 dark:text-green-200 mb-2">¡Todos los Precios Correctos!</h4>
                    <p className="text-green-700 dark:text-green-300">No se detectaron errores de precio en ninguna conversación</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {results.map((result) => {
                      const priceErrors = result.databaseActivity?.discrepancies?.filter(d => d.type === 'incorrect_data') || [];
                      if (priceErrors.length === 0) return null;
                      
                      return (
                        <div key={result.id} className="bg-white dark:bg-gray-800 rounded-xl p-6 border-2 border-red-300 dark:border-red-700">
                          <h4 className="font-bold text-lg text-red-800 dark:text-red-200 mb-4">{result.testCase.title}</h4>
                          <div className="space-y-3">
                            {priceErrors.map((error, idx) => (
                              <div key={idx} className="bg-red-50 dark:bg-red-900/20 rounded-lg p-4">
                                <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">{error.description}</p>
                                <div className="grid grid-cols-2 gap-4 mt-2">
                                  <div>
                                    <p className="text-xs text-red-600 dark:text-red-400 font-semibold">Bot Ofreció:</p>
                                    <p className="text-xl font-bold text-red-700 dark:text-red-300">${error.expected}</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-green-600 dark:text-green-400 font-semibold">Precio Real:</p>
                                    <p className="text-xl font-bold text-green-700 dark:text-green-300">${error.actual}</p>
                                  </div>
                                </div>
                              </div>
                            ))}
            ...
            {/* DATABASE TAB - Actividad de Base de Datos */}
            {activeTab === 'database' && (
              <div className="space-y-4">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-6 text-center border-2 border-blue-300">
                    <p className="text-4xl font-bold text-blue-600">{stats.totalDbOperations}</p>
                    <p className="text-sm text-blue-700 dark:text-blue-300 font-semibold mt-2">Operaciones Totales</p>
                  </div>
                  <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-6 text-center border-2 border-purple-300">
                    <p className="text-4xl font-bold text-purple-600">{stats.totalDbChanges}</p>
                    <p className="text-sm text-purple-700 dark:text-purple-300 font-semibold mt-2">Cambios Registrados</p>
                  </div>
                  <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-6 text-center border-2 border-red-300">
                    <p className="text-4xl font-bold text-red-600">{stats.totalDbErrors}</p>
                    <p className="text-sm text-red-700 dark:text-red-300 font-semibold mt-2">Errores Críticos</p>
                  </div>
                </div>
                <div className="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-6">
                  <p className="text-gray-600 dark:text-gray-400">Detalles de actividad de base de datos por conversación disponibles en la pestaña de Conversaciones</p>
                </div>
              </div>
            )}
            
            {/* CONVERSATIONS TAB - Detalle de Cada Conversación */}
